...
| R7 | Unsigned app: Gatekeeper on macOS and SmartScreen on Windows warn on first launch. | Certain | Low for a personal tool | Confirmed in v0.1.0: a CI build with no bundle signature is reported by macOS as "damaged" once quarantined, and right-click Open does not help. Since v0.1.1 the bundle is ad-hoc signed (`bundle.macOS.signingIdentity: "-"`), which yields the "Open Anyway" path in System Settings instead. README documents that path and the `xattr` alternative. Full removal needs Developer ID + notarization. |
