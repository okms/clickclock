...
- [ ] M1.1 `core/format.ts`: decimal hours, HH:MM, local day keys, day and week labels, ISO week grouping — DISP-01, DISP-03, DISP-07, HIST-02 — haiku
...
- [ ] M1.3 `core/timer.ts`: the state machine — TT-01..TT-04, TT-06..TT-11, IDLE-02..IDLE-06, IDLE-07 — sonnet (time logic is the product's main risk)

## M2: App (browser-runnable via the mock platform)
- [ ] M2.1 `app/platform/platform.ts` interface + `app/platform/mock.ts` (localStorage, simulated idle control) — haiku
...
## M3: Shell (Tauri 2)
- [ ] M3.1 `src-tauri/` scaffold with the four commands, tray, autostart and single-instance plugins, close-hides-window — TRAY-01..04, TRAY-06, TRAY-07, AUTO-*, PERS-03, PERS-04 — sonnet
...
- [ ] M4.2 README updated with build/run instructions and the Gatekeeper note
- [ ] M4.3 Spec statuses reviewed; anything not implemented is reflected in the plan
